...
use crate::models::codex_local_access::{
    ...
    CodexLocalAccessTestFailure, CodexLocalAccessTestResult,
...
fn build_state_snapshot(runtime: &GatewayRuntime) -> CodexLocalAccessState {
    ...
    let external_api_port_url = collection.as_ref().and_then(|item| {
        if item.access_scope == CodexLocalAccessScope::Lan {
            build_external_api_port_url(item.port)
        } else {
            None
        }
    });
    let external_base_url = collection.as_ref().and_then(|item| {
        if item.access_scope == CodexLocalAccessScope::Lan {
            build_external_base_url(item.port)
        } else {
            None
        }
    });
...
pub async fn update_local_access_restrict_free_models(
    ...
    if collection.restrict_free_models == normalized {
        return snapshot_state().await;
    }
